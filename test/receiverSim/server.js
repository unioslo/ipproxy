...
var server = app.listen(config.getListenPortInt(), function () {
  ...
  logStr = 'receiverSim app listening at http://' + host + ':' + port;
  console.log(logStr);
  logger.info(logStr);
});

var gracefulShutdown = function () {
  logger.info('receiverSim about to exit, waiting for remaining connections to complete');

  server.close(function () {
    logger.info('receiverSim app closed gracefully');
