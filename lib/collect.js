...
var postToAnalytics = function (
  ...
  data = new Buffer.from(queryStringified + "&" + extraStringified, "utf-8");
  ...
  postOptions = {
    ...
    method: "POST",
    headers: {
      "User-Agent": "UiOIpproxy/" + config.get("version"),
    ...
    res.on("end", function () {
      ...
      logger.info(
        "TIMES (ms) poststatus: " +
          status +
          " client: " +
          hrTimeDiffToMillis(clientResponseDiff) +
          " analytics: " +
          hrTimeDiffToMillis(postDiff) +
          " total: " +
          hrTimeDiffToMillis(totalDiff)
      );
    });
  });
...
var collect = function (req, res) {
  ...
  // referer is available in Javascript (document.referrer) and should already have been added as the dr parameter
  // by analytics.js, so don't mess with it.
